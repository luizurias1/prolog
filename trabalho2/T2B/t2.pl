...
referencia('Bruce Wayne', 'Natasha Romanov').
...
xp('Bruce Wayne', 'Bilionário', 'Wayne Enterprises', 2010, 2015).
...
write(INICIO), write(' até '), write(FIM), nl.
...
%------- TRABALHO PROLOG B PARTE 2 -----------.

%1e2)
grafo:-referencia(P,O), open('grafo.pl', append, Stream), write(Stream,P), write(Stream,' - '), write(Stream, O), nl(Stream), close(Stream).

%3)
connected(X,Y) :- referencia(X,Y).
connected(X,Y) :- referencia(Y,X).
...
%4)
tamanho(Nome1, Nome2, N, P):-path(Nome1, Nome2, P), listlen(P, N).
tamanhos(Nome1, Nome2, V):- findall(N, tamanho(Nome1, Nome2, N,_), Lista), min(Lista, V).
menorCaminho(N,M,C):-tamanhos(N,M,T),tamanho(M,N,T,C). 
